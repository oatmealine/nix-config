...
buildDotnetModule (finalAttrs: {
  pname = "periphery-export";
  version = "1.2.1";

  src = fetchFromGitHub {
    owner = "one-nora";
    repo = "PeripheryExport";
    tag = finalAttrs.version;
    hash = "sha256-uCJyWk3knjXymob6Q2eLVeJJrAPPtwSg/X223mMEWts=";
  };
  ...
  postInstall = ''
    mkdir -p $out/bin
    ln -rs $out/bin/AssetRipper.GUI.Free $out/bin/PeripheryExport
  '';
  ...
  patches = [ ./dont-save-settings.patch ];
